# Evidências: Delegated rollout observability

## Comandos executados

| Comando | Resultado | Quando |
|---|---|---|
| `python -B -m unittest discover -s tests -q` via `scripts/verify_feature.py` | 141 tests passed (2 skipped) | 2026-08-08 |
| `python scripts/delegation_contract.py specs/delegated-rollout-observability --json` | contrato v2 válido; grafo e evidência válidos | 2026-08-08 |
| `python scripts/validate_feature.py . specs/delegated-rollout-observability` | executado após geração de `verification.json` | 2026-08-08 |

## Rastreabilidade

| Critério | Implementação | Teste/evidência | Status |
|---|---|---|---|
| AC-801 | `scripts/delegation_telemetry.py` | `tests/test_delegation_telemetry.py` (`@spec:AC-801`), `verification.json` | Concluído |
| AC-802 | `scripts/delegation_telemetry.py` | `tests/test_delegation_telemetry.py` (`@spec:AC-802`), `verification.json` | Concluído |
| AC-803 | `scripts/delegation_telemetry.py` | `tests/test_delegation_telemetry.py` (`@spec:AC-803`), `verification.json` | Concluído |
| AC-804 | `scripts/model_routing.py` | `tests/test_delegation_routing.py` (`@spec:AC-804`), `verification.json` | Concluído |
...
## Riscos residuais
## Resultado da coleta delegada

`python scripts/delegation_telemetry.py collect --output specs/delegated-rollout-observability/delegation-evidence.json --manifest specs/delegated-rollout-observability/delegation-evidence.json --json` foi executado em 2026-08-08. O resultado foi `collection_status: partial`: WP-001 e WP-002 foram resolvidos com modelo efetivo `gpt-5.6-luna` e custo observável; WPs de fallback direto permaneceram `not-found`, com custo `not-available` e sem conversão para zero. O subtotal delegado observado foi `0.21421452 USD` para 2 de 7 delegações; 5 parcelas ficaram listadas em `unavailable`.

O retorno não-zero do collector é intencional quando há parcela indisponível. A evidência de contrato continua válida porque o fallback de execução foi declarado e auditado separadamente da correlação de telemetria.

## Checks não executados

Nenhum check aplicável ficou pendente. Não houve validação de UI, porque esta feature não altera UI/API.
