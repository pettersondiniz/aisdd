...
When `agent_evidence.py` receives `--agent-id` and no rollout matches the runtime's `agent_path`, it may correlate the exact terminal UUID with a uniquely matching rollout filename. The result must preserve the normal telemetry and cost calculation, expose that the resolution used a `rollout-id` fallback, and emit an explicit warning. Partial UUIDs, ambiguous matches, metadata inconsistencies and unsafe paths remain fail-closed.

Features that declare `Main-chat attribution: required` in `plan.md` must persist a closed `task-window.json` and a final `task-window-report.json` generated by `task_window.py report --final --output ...`. The validator checks these artifacts and rejects an absent, open, divergent or provisional window/report. The report labels its scope as `main-chat-orchestrator`; it never includes delegated-agent usage. A combined total is valid only when the main-chat cost and delegated-agent subtotal remain separately recorded.
...
- A fallback from `--agent-id` to an exact `rollout-id` is observable in both JSON and text output and never silently changes the selected rollout.
- A declared mandatory main-chat attribution cannot be completed with only delegated-agent evidence or an open/provisional task window.
...
- [ ] AC-523: When `--agent-id` does not match `agent_path` but its exact UUID identifies one rollout filename, the tool resolves that rollout and performs the normal model, token and cost extraction.
- [ ] AC-524: A UUID correlation fallback is explicit in JSON (`resolution.fallback_used`) and text (`AGENT_ID_FALLBACK`), including the original selector and resolved rollout ID.
- [ ] AC-525: Partial UUIDs, ambiguous rollout matches, inconsistent metadata and paths outside `--sessions-root` remain fail-closed without inventing evidence.
- [ ] AC-526: A feature whose plan declares `Main-chat attribution: required` fails validation when `task-window.json` or `task-window-report.json` is absent, open, provisional, mismatched or unsafe.
- [ ] AC-527: `task_window.py report --final --output ...` rejects an open window and writes a final report only for a closed window with a stable session/boundary identity.
- [ ] AC-528: A final task-window report identifies `main-chat-orchestrator` as its scope and excludes delegated-agent rollouts, tools and subscription billing.
- [ ] AC-529: Evidence validation keeps delegated-agent subtotal, main-chat cost and combined cost separate; unavailable cost is reported as unavailable and never converted to zero.

## Decisions

- D-515: Prefer an exact rollout UUID fallback over `agent_path` only after direct `--agent-id` correlation fails, and expose the fallback as an alert.
- D-516: Make main-chat attribution enforceable through a plan declaration plus closed sidecar/report artifacts, without mixing orchestrator and delegated-agent usage.
